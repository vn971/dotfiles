...
fn main() {
	...
	for display_file in display_list {
		...
		if window_name.contains(&display_name)
			...
			let secondary = Command::new("xclip")
					.arg("-selection").arg("XA_SECONDARY").arg("-o")
					.output().unwrap().stdout;
			let secondary = String::from_utf8_lossy(&secondary);
			println!("Secondary selection content: {:?}", secondary);
			...
			let put_command = Command::new("xclip")
					...
					.stdin(Stdio::piped())
					.spawn().unwrap();
			write!(put_command.stdin.unwrap(), "{}", secondary).unwrap();
		}
	}
}
